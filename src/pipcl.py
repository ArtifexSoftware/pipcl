...
Overview:
    ...
    For example we provide a function `pipcl.build_extension()` that can be used
...
class Package:
    ...
    The constructor takes a definition of a Python package similar to that
    ...
    Methods are provided that can be used to implement a Python package's
    `setup.py` supporting PEP-517.

    Basic command line handling is also supported for
    use with a legacy (pre-PEP-517) pip, as implemented
    ...
    module called 'foo', with a console-script command line tool called
    `cli`. Requires `swig`.
    ...
    Wheels and sdists
        ...
        Wheels:
            ...
            * Other `RECORD*` files such as `RECORD.jws` or `RECORD.p7s` are
              not generated.
    ...
    def __init__(self,
        ...
        Args:
            ...
            entry_points:
                A string specifying *.dist-info/entry_points.txt, for
                example::
                ...
                Alternatively one can specify a dict similar to that supported
                by setuptools (see
            ...
            fn_build:
                ...
                arg (as described in PEP-517). This should build all required
                files and return a list of items; each item should be a tuple
        ...
        assert_str_or_multi( requires_python)
    ...
    def tag_abi(self):
        ...
        if self.tag_abi_:
            ...
            assert Py_GIL_DISABLED != 1, \
                    f'{self.py_limited_api=} and {Py_GIL_DISABLED=} are not supported together as of 2026-02-20, e.g. see PEP 803 and PEP 809.'
            return 'abi3'
    ...
    def _call_fn_build( self, config_settings=None):
        ...
        assert os.path.isfile('pyproject.toml'), (
                f'Cannot create package because file does not exist: {os.path.abspath("pyproject.toml")}'
                )
    ...
    def _metainfo(self):
        ...
        ret = ['']
        def add(key, value):
            if value is None and key == 'Description-Content-Type':
                # Try to figure out from description suffix.
                if self.description and '\n' not in self.description:
                    if self.description.endswith('.md'):
                        value = 'text/markdown'
                    elif self.description.endswith('.rst'):
                        value = 'text/x-rst'
                    if value:
                        log(f'## Warning: description_content_type unspecified, inferring {value!r} from suffix of description={self.description!r}.')
            if value is None:
                return
...
def build_extension(
    ...
    if windows():
        ...
        command += f'    /DLL\n'
        command += f'    /EXPORT:PyInit__{name}\n'
        command += f'    /IMPLIB:{base}.lib      # Overrides the default import library name.\n'
    else:
        command += f'    -MD -MF {prerequisites_path}\n'
        command += f'    -shared\n'
...
def base_compiler(vs=None, pythonflags=None, cpp=False, use_env=True):
    ...
    Args:
        ...
        use_env:
            ...
            environmental variable is set (without evaluating with `os.getenv()`
            or `os.environ`).
...
def linker_command(
    ...
    linker_command, pythonflags = base_linker(cpp=cpp)
    
    if windows():
        debug2 = '/Zi' if debug else ''
        py_limited_api3 = f'/DPy_LIMITED_API={py_limited_api2}' if py_limited_api2 else ''
        debug2 = '/DEBUG' if debug else ''
        linker_command = textwrap.dedent(f'''
                {linker_command}
                    {libpaths_text}
        ...
        t = textwrap.dedent(f'''
                ...
                {py_limited_api3}
                ''')
        linker_command += textwrap.indent(t, '    ')
    return linker_command


def macos_patch2(path_so, libpaths, libs):
    assert darwin()
...
def fs_read(path, binary=False):
    '''
    Returns string containing contents of file <path>. Returns bytes if
    <binary> is true.
    '''
...
def git_get(
    ...
    if ':' not in remote:
        # Allow clone/fetch from local directory.
        remote = os.path.abspath(remote)
    
    try:
        # Convert local remotes to absolute path so it still works after we've
        # cd-ed into local directory.
        #
        if ':' not in remote:
            remote_0 = remote
            remote = os.path.abspath(remote)
            log(f'Changed local remote {remote_0!r} to {remote=}')
    ...
    try:
        ...
        if prefix or ticker or out!=[sys.stdout]:
            ...
            while 1:
                ...
                if is_windows:
                    # Presence of \r bytes on windows causes them to
                    # proliferate in nested runs, leading to logs being
                    # difficult to read. So we remove them explicitly. (Our
                    # subprocess.Popen instance will probably have text mode
                    # stdout/stderr, but we read from child.stdout.fileno()
                    # which will always be binary.)
                    #
                    text = text.replace('\r\n', '\n')
                if text:
...
def _assert_version_pep_440(version):
    '''
    Asserts that <version> cconforms with PEP 440.
    '''
    assert re.match(
...
def log(text='', caller=1):
    '''
    Writes <text> to log. Caller identifies the frame to use if file:line is
    included in prefix; default 1 identifies the caller.
    '''
